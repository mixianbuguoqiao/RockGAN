...
LOCAL_PATH = 'E:/仓库/RockGAN/volumetric_data/'
SERVER_PATH = 'E:/仓库/RockGAN/volumetric_data/'
...
def getAll(obj='airplane', train=True, is_local=False, cube_len=64, obj_ratio=1.0):
    ...
    if os.path.exists('data/train_data.db.npy'):
        return np.load('data/train_data.db.npy')
    volumeBatch = Voxel.importVoxByPath('.\\ObjData\\voxs_64\\', 64)
    return volumeBatch

def getAllchair(obj='airplane',train=True, is_local=False, cube_len=64, obj_ratio=1.0):
    objPath = SERVER_PATH + obj + '/30/'
    if is_local:
        objPath = LOCAL_PATH + obj + '/30/'
    objPath += 'train/' if train else 'test/'
...
def getBounds():
    if os.path.exists('data/BoundList.npy'):
        bound_list = np.load('data/BoundList.npy')
